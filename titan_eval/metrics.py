...
from titan_eval.utils import load_references
from titan_eval.database import MultiWOZVenueDatabase
from titan_eval.normalization import normalize_data

from titan_eval.utils import has_domain_predictions, get_domain_estimates_from_state
from titan_eval.utils import has_state_predictions
from titan_eval.utils import load_goals, load_booked_domains, load_gold_states
...
class Evaluator:
    ...
    def evaluate(self, input_data):
        ...
        return {
            ...
            "success": get_success(input_data, self.database, self.goals,
                                   self.booked_domains) if self.success else None,
...
def get_dialog_success(goal, booked_domains, utterances, states, domain_estimates, database):
    ...
    for system_utterance, state, booked_domain, domain_estimate in zip(utterances, states, booked_domains,
                                                                       domain_estimates):
        ...
        for current_domain in goal:
            ...
            if ('NAME' in system_utterance and current_domain in ['restaurant', 'hotel', 'attraction']) or (
                    'TRAINID' in system_utterance and current_domain == 'train'):

                # The INFORM rate metric takes into account just the *last* mention about the particular venue into account
                matching_venues = database.query(current_domain,
                                                 state[current_domain]) if current_domain in state else []
